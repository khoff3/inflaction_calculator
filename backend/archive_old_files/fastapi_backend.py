...
app = FastAPI(
    title="Fantasy Football Inflation Calculator API",
    description="FastAPI backend for calculating fantasy football auction inflation rates",
    version="2.0.0"
)
...
async def get_draft_data_async(draft_id: str) -> List[Dict[str, Any]]:
    """Async version of get_draft_data using thread pool."""
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(executor, get_draft_data, draft_id)

def get_draft_data(draft_id: str) -> List[Dict[str, Any]]:
...
def get_player_info(player_name: str, position: str) -> tuple:
    """Get player auction value and tier information."""
...
def map_players_to_ev_data(draft_data: List[Dict[str, Any]]) -> tuple:
    """Map draft players to expected value data."""
    # Merge player data with auction values data
    all_data = pd.concat([positional_rankings['QB'], positional_rankings['RB'], 
                         positional_rankings['WR'], positional_rankings['TE']], ignore_index=True)
...
def get_picks_per_tier(draft_data: List[Dict[str, Any]], expected_values: pd.DataFrame) -> Dict[str, Dict[str, int]]:
    """Calculate picks per tier for each position."""
...
def get_avg_tier_cost(draft_data: List[Dict[str, Any]], expected_values: pd.DataFrame) -> Dict[tuple, float]:
...
def calculate_inflation_rates(draft_data: List[Dict[str, Any]]) -> tuple:
    ...
    if 'Tier' not in expected_values.columns:
        ...
        return {"overall": 0, "positional": {}, "positional_tiered": {}, "avg_tier_costs": {}, "expected_values": []}, expected_values
...
def calculate_r2_by_position(draft_data: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    """Calculate R² values by position."""
    position_r2 = {}
...
def calculate_doe_values(draft_data: List[Dict[str, Any]], expected_values: pd.DataFrame, positional_tier_inflation: Dict[str, Dict[str, float]]) -> Dict[str, Dict[str, float]]:
    """Calculate DOE (Dollar Over Expected) values."""
    doe_values = {}
    player_counts = {}
...
def calculate_avg_tier_costs(expected_values: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    """Calculate average tier costs."""
    avg_tier_costs = {}
...
def calculate_team_strengths_and_needs_by_tier(team_data: Dict[str, Any], expected_values: pd.DataFrame) -> Dict[str, Dict[str, str]]:
    """Calculate team strengths and needs by tier."""
    strengths_and_needs = {}
...
def process_team_breakdown(draft_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Process team breakdown from draft data."""
    team_data = {}
...
async def root():
    ...
    return {
        "message": "Fantasy Football Inflation Calculator API",
        "version": "2.0.0",
...
async def health_check():
    ...
    return {"status": "healthy", "latest_year": LATEST_YEAR}
...
async def team_breakdown(draft_id: str = Query(..., description="Sleeper draft ID")):
    ...
    try:
        draft_data = await get_draft_data_async(draft_id)
        if not draft_data:
        ...
        raise HTTPException(status_code=500, detail=f"KeyError: Missing key in draft data - {e}")

    except ValueError as e:
        logging.error(f"ValueError: Invalid data format - {e}")
        raise HTTPException(status_code=500, detail=f"ValueError: Invalid data format - {e}")
...
async def scatter_data(draft_id: str = Query(..., description="Sleeper draft ID")):
    """Get scatter plot data for a draft."""
    try:
        draft_data = await get_draft_data_async(draft_id)
        if not draft_data:
            raise HTTPException(status_code=404, detail="No draft data found")
...
async def get_picks(draft_id: str = Query(..., description="Sleeper draft ID")):
    """Get raw picks data for a draft."""
    draft_data = await get_draft_data_async(draft_id)
    if not draft_data:
        raise HTTPException(status_code=404, detail="No draft data found")
...
async def player_lookup(request: PlayerLookupRequest):
    """Look up player auction values and tiers."""
    try:
        results = []
...
async def get_inflation_rate(draft_id: str = Query(..., description="Sleeper draft ID")):
    """Get inflation rate data for a draft."""
    try:
        draft_data = await get_draft_data_async(draft_id)
        if not draft_data:
            raise HTTPException(status_code=404, detail="No draft data found")
